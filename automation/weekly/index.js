...
// Get the date of two Saturdays ago
let twoSaturdaysAgo = new Date();
twoSaturdaysAgo.setDate(currentDate.getDate() - currentDate.getDay() - 7);

// Get the date of last Saturday
let lastSaturday = new Date();
lastSaturday.setDate(currentDate.getDate() - currentDate.getDay());

// Format the dates in YYYY-MM-DD format
let twoSaturdaysAgoStr = twoSaturdaysAgo.toISOString().split('T')[0];
let lastSaturdayStr = lastSaturday.toISOString().split('T')[0];

// Function to fetch data for each author
async function fetchData(author) {
    const response = await fetch(`https://api.github.com/search/issues?q=author:${author}+created:${twoSaturdaysAgoStr}..${lastSaturdayStr}`);
...
// month and year
console.log(`# GitHub issues and prs from ${twoSaturdaysAgo.toLocaleString('default', { month: 'long' })} ${twoSaturdaysAgo.getDate()}, ${twoSaturdaysAgo.getFullYear()} to ${lastSaturday.toLocaleString('default', { month: 'long' })} ${lastSaturday.getDate()}, ${lastSaturday.getFullYear()}`);
