...
export default class GitHubRepoIssues {
    static async fetchIssues(params: FetchIssuesParams, accessToken: string): Promise<GitHubIssue[]> {
        const { repo, dateRange/*, state, labels, assignee, creator, mentioned, sort, direction*/ } = params;
        ...
        if (!response.ok) {
            console.log('GitHubRepoIssues.fetchIssues', { status: response.status, statusText: response.statusText });
