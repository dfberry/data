...
function queryParts(query) {
    // example is https://api.github.com/repos/Azure-Samples/js-e2e-express-server/issues?created:2024-05-23..2024-05-23
...
function gitHubUrlToParts(url) {
    ...
    //const url = 'https://api.github.com/repos/Azure-Samples/azure-search-openai-demo';
    const regex = /https:\/\/api\.github\.com\/repos\/([^\/]+)\/([^\/]+)/;
...
function step1() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            // data in GitHub
            githubFiles = yield getFilesInGitHubSubdir(user, repo, 'github/stats');
        ...
        try {
            ...
            try {
                ...
                if (filesToDownload.length === 0)
                    return;
                const downloadFileList = yield downloadFileFromGitHubManager(filesToDownload);
        ...
        try {
            ...
            for (const file of filesToInsertIntoDb) {
                ...
                for (const item of content) {
                    ...
                    item.id = `${parts.endpoint}.${parts.queryParam}.${urlParts.orgName}.${urlParts.repoName}.${parts.startDate}.${item.original_id}`;
                    console.log(`Inserting item ${item.id}`);
                    item.type = fileNameParts === null || fileNameParts === void 0 ? void 0 : fileNameParts.name;
